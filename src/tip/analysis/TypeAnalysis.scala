    ...
    node match {
      case program: AProgram =>
        unify(
          ...
          FunctionType(program.mainFunction.params.map(_ => IntType(): Term[Type]), IntType())
        )
      ...
      case as: AAssignStmt =>
        as.left match {
          ...
          case dw: ADerefWrite => unify(PointerType(as.right), dw.exp)
          case dfw: ADirectFieldWrite =>
            unify(
              dfw.id,
              RecordType(allFieldNames.map { f => if (f != dfw.field) FreshVarType() else VarType(as.right) })
            )
          case ifw: AIndirectFieldWrite =>
            unify(
              ifw.exp,
              RecordType(allFieldNames.map { f => if (f != ifw.field) FreshVarType() else VarType(as.right) })
            )
        }
      case bin: ABinaryOp =>
        bin.operator match {
